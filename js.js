function addSection() {
    ...
    container.colSpan = numContainers;
    ...
    for (let i = 0; i < numContainers; i++) {
        ...
        cell.style.flex = '1';
        cell.innerHTML = `<a href="${link}" target="_blank"><img src="${image}" alt="Image ${i+1}"></a>`;
...
document.getElementById('numContainers').addEventListener('input', function() {
    ...
    for (let i = 0; i < numContainers; i++) {
        ...
        linkInput.type = 'url';
        linkInput.id = `link${i}`;
        linkInput.placeholder = `Link URL for Container ${i + 1}`;
        containerInputs.appendChild(linkInput);

        const imageInput = document.createElement('input');
        imageInput.type = 'url';
        imageInput.id = `image${i}`;
        imageInput.placeholder = `Image URL for Container ${i + 1}`;
        containerInputs.appendChild(imageInput);
    }
});

function copyTable() {
    const tableContainer = document.querySelector('.table-container');
    const removeButtons = Array.from(tableContainer.querySelectorAll('.remove-btn'));

    const buttonParents = removeButtons.map(button => button.parentNode);
    removeButtons.forEach(button => button.parentNode.removeChild(button));

    const tableHTML = tableContainer.innerHTML;

    removeButtons.forEach((button, index) => {
        buttonParents[index].appendChild(button);
    });
